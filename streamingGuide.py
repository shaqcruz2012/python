# First, write a class named **Movie** that has four data members: **title**, **genre**, **director**, and **year**. 
# It should have:
    ...
    def where_to_watch_movie(self, title):
        ...
        for service in self.__services:
            if title in service.get_catalog():
                services_list.append(service.get_name())
        if len(services_list)>0:
            movie = service.get_catalog()[title]
            services_list.insert(0, f"{movie.get_title()} ({movie.get_year()})")
